...
import { useRoqClient, useJobsFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<JobsInterface, unknown>;
...
export function JobsListPage(props: JobsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useJobsFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.jobs.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: JobsInterface) => {
    if (hasAccess('jobs', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('jobs', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('jobs', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="jobs" />
        </Flex>

        {hasAccess('jobs', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
