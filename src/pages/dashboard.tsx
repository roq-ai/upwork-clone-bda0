...
import { useRoqPlatformUserProfiles, useJobsCount, useApplicationsCount } from 'lib/roq';

import { JobsListPage as ShowCaseList } from 'pages/jobs';

function DashboardPage() {
  const { session } = useSession();

  const jobsStat = useJobsCount();

  const applicationsStat = useApplicationsCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Jobs',
      result: jobsStat?.data,
      isLoading: jobsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Applications',
      result: applicationsStat?.data,
      isLoading: applicationsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
